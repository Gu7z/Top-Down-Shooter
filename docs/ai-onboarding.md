...
1. `src/menu.js` renders the start screen. Before the player interacts with the menu, it may show the current patch notes modal from `src/patch_notes/*` if the browser has not acknowledged the current patch content hash yet. After the player types a username and starts, `src/game.js` constructs the engine and hands control to `WaveManager`.
...
- **Menu & patch notes** (`src/menu.js`, `src/patch_notes/*`): own the first-run entry flow, username capture, main navigation buttons, and the patch-notes acknowledgment modal that is suppressed per-browser via `localStorage` until the patch content changes.
